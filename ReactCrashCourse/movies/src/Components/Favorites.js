...
export default class Favorites extends Component {
    ...
    sortPopularityDesc = () =>{
        ...
        temp.sort( function(movObjA, movObjB){
            return movObjB.popularity - movObjA.popularity
        })
        this.setState({
            movies :[...temp]
        })
    ...
    sortPopularityAsc = () =>{
        let temp = this.state.movies;
        temp.sort( function(movObjA, movObjB){
            return movObjA.popularity - movObjB.popularity
        })
        this.setState({
            movies :[...temp]
        })
    ...
    sortRatingDesc = () =>{
        let temp = this.state.movies;
        temp.sort( function(movObjA, movObjB){
            return movObjB.vote_average - movObjA.vote_average
        })
        this.setState({
            movies :[...temp]
        })
    ...
    sortRatingAsc = () =>{
        let temp = this.state.movies;
        temp.sort( function(movObjA, movObjB){
            return movObjA.vote_average - movObjB.vote_average
        })
        this.setState({
            movies :[...temp]
        })
            ...
            <div className='main'>
                <div className='row'>
                    ...
                    <ul class="list-group favourites-genres">
                        {
                            this.state.genres.map( (genre) =>(
                                ...
                                :  
                                <li class="list-group-item" onClick={ () => this.hanldeCurrGenre(genre)}>{genre}</li>
                            ))
                        }
                    </ul>
                    </div>
                    <div className='col-9 favourites-table'>
                        <div className='row'>
                        <input type="text" className="form-control col" placeholder="Search" value = {this.state.currText} onChange={(e) => this.setState({
                            currText : e.target.value
                        })} />
                        <input type="number" className="form-control col" placeholder="Rows Count" value = {this.state.limit} onChange={(e) => this.setState({ limit : e.target.value})} />
